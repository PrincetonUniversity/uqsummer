...
void readXMLModelInput(RefPtr<XMLElement> xmlTree,Array1D<double>& modelparams,Array1D<string>& modelparamnames,Array1D<double>& modelauxparams,dumpInfo* outPrint);
void readXMLDumpInfo(RefPtr<XMLElement> xmlTree, int* dumpInt, int* fdumpInt, string* dumpfile);
...
void readXMLModelInput(RefPtr<XMLElement> xmlTree, Array1D<double>& modelparams,Array1D<string>& modelparamnames,Array1D<double>& modelauxparams )
{
  ...
  RefPtr<XMLAttributeList> integSettings = xmlTree->get_child("RunSettings")->get_child("IntegrationParams")->attributes();
  double t1 = integSettings->get_double("end",1e3);
  double dTym = integSettings->get_double("step",1.e-2);
  modelauxparams.Resize(integSettings->size());
  modelauxparams(0)=t1;
  modelauxparams(1)=dTym;
...
void readXMLDumpInfo(RefPtr<XMLElement> xmlTree, int* dumpInt, int* fdumpInt, string* dumpfile)
{
 ...
 // Get IO Settings
  RefPtr<XMLAttributeList> ioSettings = xmlTree->get_child("RunSettings")->get_child("IOParams")->attributes();
  *dumpInt = ioSettings->get_int("screenIOInt",10000); // Interval in time steps for writing to screen
  *fdumpInt = ioSettings->get_int("fileIOInt",1);      // Interval in time steps for writing to file
  *dumpfile = ioSettings->get("fileName","solution.dat"); // Output file name
...
void readXMLChainInput(RefPtr<XMLElement> xmlTree,MCMC* pmchain, Array1D<double>& chstart, int* nsteps, Array1D<int>& chainParamInd, Array1D<string>& priortype, Array1D<double>& priorparam1, Array1D<double>& priorparam2)
{
  ...
  Array1D<int> adaptstep(3,0);
  adaptstep(0)=pt1->attributes()->get_int("adstart",1000);
  adaptstep(1)=pt1->attributes()->get_int("adstep",10);
  adaptstep(2)=pt1->attributes()->get_int("adstop",1000000);
  ...
  pmchain->initAMParams(gamma,eps_cov,adaptstep);
