/* =====================================================================================
   ...
   DE-AC04-94AL85000 with Sandia Corporation, there is a non-exclusive
...
/// Main program of uncertainty propagation of the ODE model parameters via non-intrusive spectral projection (NISP)
int main()
{
	// Model parameters
	...
	// Initial conditions cannot be at the origin, or nothing will happen.
	Array1D<double> u(nPCTerms,1.e0);
	Array1D<double> v(nPCTerms,1.e0);
	Array1D<double> w(nPCTerms,1.e0);

	// Evaluation the model at quadrature points
	Array2D<double> allmodelparams(nQdpts,fulldim,0.e0);
	for(int iq=0;iq<nQdpts;iq++){
		...
		for(int idim=0;idim<dim;idim++){
			...
			modelparams(uncParamInd(idim))=  inpPC.EvalPC(cur_cf, qdpt);
		}

		printf("Model parameters for sample %d / %d: ", iq+1, nQdpts);
	...
	Array1D<double> uu(nQdpts,1.e0),vv(nQdpts,1.e0),ww(nQdpts,1.e0);
