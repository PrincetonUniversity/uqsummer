...
from prob3_utils import funcBB, plotKdeBin, plotFunQdpts
...
if options.nomvals is None:
    ...
    nom=npy.array([int(x.strip()) for x in dsize])
...
if npc != npcc:
    print "prob3(): The number of input PC coefficients does not match to the given dimension and order -> Abort !\n"
    quit()
...
print "     ",inpceval.replace(uqtkbin+"/",""),"->",
...
#plotFunQdpts("input_fcn.dat","output_fcn.dat","input.dat","output.dat","prob3_model.pdf")

# 4. build a PC expansion for the output
prspcmd = pcrsp+" -x"+pcType+" -o"+str(rord)+" -d"+str(dim)+" -e > logPCEresp.dat"
print "     ",prspcmd.replace(uqtkbin+"/",""),"->",
os.system(prspcmd)
...
pcrvcmd=pcerv+" -w'PCvar' -x"+pcType+" -d"+str(dim)+" -n200 -p"+str(dim)+" -q0 > logPCrv.dat"
print "     ",pcrvcmd.replace(uqtkbin+"/",""),"->",
os.system(pcrvcmd)
...
print "     ",outpceval.replace(uqtkbin+"/",""),"->",
for k in range(indim_par):
    npy.savetxt("pccf.dat",pccf[:,k],fmt='%.18e', delimiter=' ', newline='\n')
    os.system(outpceval)
...
#--------------------------------------------------------------------------------------------------
pcerv=uqtkbin+"/pce_rv"
splcmd=pcerv+" -w'PC' -f'PCcoeff_quad.dat' -x'HG' -d1 -n"+str(npces)+" -p"+str(dim)+" -q"+str(rord)+" > logPCEspl.dat"
print "     ",splcmd.replace(uqtkbin+"/",""),"->",
os.system(splcmd)
shutil.move("rvar.dat","PCEspls_"+str(npces/1000)+"k.dat")
print " Done !"
print "      plot pdf"
plotKdeBin("PCEspls_"+str(npces/1000)+"k.dat","prob3_PCEdens_"+str(npces/1000)+"k.pdf",npdf=npdf)
print "-----------------------------------------------------------------------------------------------------"
